...
class IsometricProjection:
    def __init__(self):
        """
        Получение пар индексов для отрисовки параллелепипеда,
        Генерация матрицы поворота для трехмерного многоугольника,
        Генерация матрицы для получения проеции (после отсечения z координаты)
        """
        self.polygon = None
    ...
    def init_polygon_function(self, obj: SimpleVisualizer):
        """
        Создание прямоугольного параллелепипеда со размерами сторон - x_len, y_len, z_len и его отрисовка.
        :param obj: объект визуализатора, в котором происходит отрисовка
        """
    ...
    def _get_pairs():
        """
        Получение пар индексов, по которым можно получить все стороны параллелепипеда.
        """
        pairs = []
        for i in range(7):
            pairs.append((i, i + 1))
    ...
    def turn_polygon(self):
        """
        Поворот трехмерного параллелепипеда при помощи матрицы поворота в плоскости Оxy
        """
    ...
    def draw_polygon(self, polygon_2d: np.array, vis: SimpleVisualizer):
        """
        Отрисовка проекции
        :param polygon_2d: массив с точками
        :param vis: объект визуализатора, в котором происходит отрисовка.
        """
        vis.canv.delete("all")
    ...
    def make_projection(self) -> np.array:
        ...
        # дополнительный сдвиг необходим для центрирования многоугольника
        return projection

    def action_decor(self, vis: SimpleVisualizer):
        """
        Основная функция, которая вызывается при нажатии кнопки Run.
        """
    ...
    def _get_turn_mtrx() -> np.array:
        """
        Функция получения матрицы поворота вокруг оси Oz
        """
        return np.array([
    ...
    def _get_proj_mtrx() -> np.array:
        """
        Функция получения матрицы преобразования для получения проекции
        """
